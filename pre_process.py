...
def make_unique_response(responses: pd.DataFrame, le: sklearn.preprocessing.LabelEncoder) -> pd.DataFrame:
    ...
    le.fit(responses)
    responses = le.transform(responses)
    return responses
...
def run_preprocess(samples_file_name: str, responses_file_name: str, cols_to_remove: [str], cols_to_dummies: [str],
                   mode: str = None, le: sklearn.preprocessing.LabelEncoder = None):
    """
    return matrix of only numbers
    """
    ...
    if mode == 'meta':
        y_train = make_unique_response(y_train, le)
    for col in convert_to_mean:
        X_train[col].fillna(X_train[col].mean())
